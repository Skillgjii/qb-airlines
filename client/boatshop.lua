            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    ...
                    if IsControlJustPressed(0, 161) or IsDisabledControlJustReleased(0, 161) then
                        TriggerServerEvent('qb-airlines:server:BuyBoat', QBBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('qb-airlines:client:BuyBoat')
AddEventHandler('qb-airlines:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
